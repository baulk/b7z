...
EXTERN_C PIDLIST_ABSOLUTE WINAPI ModernSHBrowseForFolderW(LPBROWSEINFOW uType) {
  ...
  if (FAILED(SHCreateItemFromParsingName(reinterpret_cast<PCWSTR>(uType->lParam), nullptr, IID_IShellItem,
                                         defaultFolder.put_void()))) {
    return nullptr;
  }
...
//     __identifier("_imp__SHBrowseForFolderW@4") = reinterpret_cast<void const *>(::ModernSHBrowseForFolderW);
...
extern "C" __declspec(selectany) void const *const __imp_SHBrowseForFolderW =
    reinterpret_cast<void const *>(::ModernSHBrowseForFolderW);
